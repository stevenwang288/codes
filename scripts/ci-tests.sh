...
BIN="${CI_CLI_BIN:-}"
if [[ -z "${BIN}" ]]; then
  if [[ -x ./code-rs/target/dev-fast/codes ]]; then
    BIN=./code-rs/target/dev-fast/codes
  elif [[ -x ./code-rs/target/debug/codes ]]; then
    BIN=./code-rs/target/debug/codes
  fi
fi
