...
fn init_language_from_env() {
    LANGUAGE_INIT.get_or_init(|| {
        for var in ["CODE_LANG", "CODES_LANG"] {
...
fn env_path_non_empty(var: &str) -> Option<PathBuf> {
...
fn resolve_code_home_for_write() -> Option<PathBuf> {
    if let Some(path) = env_path_non_empty("CODE_HOME") {
        return Some(path);
    }
    if let Some(path) = env_path_non_empty("CODEX_HOME") {
        return Some(path);
    }
...
fn resolve_code_home_candidates_for_read() -> Vec<PathBuf> {
    ...
    if let Some(home) = resolve_user_home() {
        candidates.push(home.join(".code"));
...
fn missing_log_path() -> Option<PathBuf> {
    let base = resolve_code_home_for_write()?;
    Some(base.join("i18n-missing.jsonl"))
}

fn read_language_from_file() -> Option<Language> {
    for base in resolve_code_home_candidates_for_read() {
        let path = base.join("ui-language.txt");
        let raw = match std::fs::read_to_string(&path) {
            Ok(raw) => raw,
            Err(_) => continue,
        };
        if let Some(lang) = parse_language(raw.trim()) {
            return Some(lang);
        }
    }
