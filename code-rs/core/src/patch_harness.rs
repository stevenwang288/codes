...
pub fn run_patch_harness(
    ...
    // Apply the patch as a temporary overlay in the *real* workspace so tools can
    // discover configs/imports without requiring us to copy the full repository.
    //
    // The overlay is reverted on drop, keeping the harness side-effect free with
    // respect to the patched files.
    let overlay = WorkspaceOverlay::apply(action).ok()?;

    let mut changed_paths: Vec<PathBuf> = Vec::new();
    for (path, change) in action.changes() {
        let dest_path = match change {
            ApplyPatchFileChange::Add { .. } => Some(path.as_path()),
            ApplyPatchFileChange::Update { move_path, .. } => {
                Some(move_path.as_ref().map_or(path.as_path(), |p| p.as_path()))
            }
            ApplyPatchFileChange::Delete { .. } => None,
        };
        if let Some(dest_path) = dest_path {
            if let Ok(relative) = dest_path.strip_prefix(cwd) {
    ...
    let shell_scripts: Vec<PathBuf> = changed_paths
        .iter()
        .filter(|path| is_shell_script(cwd, path))
        .cloned()
        .collect();
                ...
                match run_with_timeout(cmd, rust_timeout) {
                    ...
                    None => {
                        findings.push(HarnessFinding {
                            tool: format!("cargo-check({label})"),
                            file: None,
                            message: format!("cargo check timed out after {rust_timeout} second(s)"),
                        });
                    }
                }
            }
    ...
    let result = if findings.is_empty() && ran.is_empty() {
        None
    } else {
        Some((findings, ran))
    };
...
fn is_shell_script(cwd: &Path, relative: &Path) -> bool {
