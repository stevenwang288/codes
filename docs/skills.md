...
CODES can automatically discover reusable "skills" you keep on disk. A skill is a small bundle with a name, a short description (what it does and when to use it), and an optional body of instructions you can open when needed. CODES injects only the name, description, and file path into the runtime context; the body stays on disk.

## Enable skills

Skills are behind the experimental `skills` feature flag and are enabled by default.

- Disable in config (preferred): add the following to `$CODES_HOME/config.toml` (usually `~/.codes/config.toml`) and restart CODES:
...
- Override for a single run when disabled in config: launch CODES with `codes --enable skills`

## Where skills live

- Location (v1): `~/.codes/skills/**/SKILL.md` (recursive). Hidden entries and symlinks are skipped. Only files named exactly `SKILL.md` count.
...
- If valid skills exist, CODES appends a runtime-only `## Skills` section after `AGENTS.md`, one bullet per skill: `- <name>: <description> (file: /absolute/path/to/SKILL.md)`.
...
1. Create `~/.codes/skills/<skill-name>/`.
2. Add `SKILL.md`:
...
4. Restart CODES to load the new skill.

## Example

```
mkdir -p ~/.codes/skills/pdf-processing
cat <<'SKILL_EXAMPLE' > ~/.codes/skills/pdf-processing/SKILL.md
---
name: pdf-processing
