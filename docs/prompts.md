...
- Location: Put files in `$CODES_HOME/prompts/` (defaults to `~/.codes/prompts/`).
...
- How to use:
  - Start a new session (CODES loads custom prompts on session start).
...
- Notes:
  ...
  - New or changed files are discovered on session start. If you add a new prompt while CODES is running, start a new session to pick it up.
