...
CODES supports several mechanisms for setting config values:
...
- A generic `-c`/`--config` flag that takes a `key=value` pair, such as `--config model="o3"`.
  ...
  - For consistency with `config.toml`, values are a string in TOML format rather than JSON format, so use `key='{a = 1, b = 2}'` rather than `key='{"a": 1, "b": 2}'`.
    - The quotes around the value are necessary, as without them your shell would split the config argument on spaces, resulting in `codes` receiving `-c key={a` with (invalid) additional arguments `=`, `1,`, `b`, `=`, `2}`.
...
- The `$CODES_HOME/config.toml` configuration file. `CODES_HOME` defaults to `~/.codes`. (Logs and other state use the same directory.)
...
Should be represented as follows in `~/.codes/config.toml`:
...
# https://github.com/stevenwang288/codes/blob/main/code-rs/rmcp-client/src/utils.rs#L82
env = { "API_KEY" = "value" }
```
...
or enable mutual TLS. Relative paths are resolved against `~/.codes/`:
...
If you build CODES from source the OTEL crate is still behind an `otel` feature
...
Specify a program that will be executed to get notified about events generated by CODES. Note that the program will receive the notification argument as a string of JSON, e.g.:
...
To have CODES use this script for notifications, you would configure it via `notify` in `~/.codes/config.toml` using the appropriate path to `notify.py` on your computer:

```toml
notify = ["python3", "/Users/mbolin/.codes/notify.py"]
```

> [!NOTE]
> Use `notify` for automation and integrations: CODES invokes your external program with a single JSON argument for each event, independent of the TUI. If you only want lightweight desktop notifications while using the TUI, prefer `tui.notifications`, which uses terminal escape codes and requires no external program. You can enable both; `tui.notifications` covers in‑TUI alerts (e.g., approval prompts), while `notify` is best for system‑level hooks or custom notifiers. Currently, `notify` emits only `agent-turn-complete`, whereas `tui.notifications` supports `agent-turn-complete` and `approval-requested` with optional filtering.

## history

By default, the CODES CLI records messages sent to the model in `$CODES_HOME/history.jsonl`. On UNIX, the file permissions are set to `o600`, so it should only be readable and writable by the owner.
