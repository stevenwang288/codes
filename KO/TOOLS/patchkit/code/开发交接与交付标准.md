...
- **PatchKit**：本仓库内的“本地补丁包与工作流工具”，目录固定为：`KO/TOOLS/patchkit/code/`。
...
3) 所有本地增强必须收敛到 `KO/TOOLS/patchkit/code/`，避免散落。

## 1. 目录结构（统一入口）

所有本地增强都放在：`KO/TOOLS/patchkit/code/`

- `KO/TOOLS/patchkit/code/patchkit.ps1`
  - 单一入口命令（建议只记这一个）。
- `KO/TOOLS/patchkit/code/scripts/`
  - 可复用的子脚本（i18n 向导、应用补丁、build、start、watchdog 等）。
- `KO/TOOLS/patchkit/code/notify/`
  - Windows 通知/声音脚本。
- `KO/TOOLS/patchkit/code/patches/`
  - 可重复回放的补丁（`git apply`）。
...
核心原则：**所有状态/配置/历史/采集日志，默认落在用户目录 `~/.codes/`（可通过 `CODES_HOME` 覆盖）**。

说明：

- `./.codes-home/` 仅作为“仓库内构建缓存/中间产物”的历史遗留目录（可清理，代价是下次编译更慢）。
- CODES 运行时不再依赖 `CODE_HOME` / `CODEX_HOME` 这类历史变量，避免与上游/其它 fork 互相污染。
...
- 默认：`~/.codes/patchkit/i18n-missing.jsonl`
...
2) 当 `zh-CN` 缺 key 时，能稳定写入 `~/.codes/patchkit/i18n-missing.jsonl`。
...
1) 所有本地改动都能通过 `KO/TOOLS/patchkit/code/patches/*.patch` 回放。
