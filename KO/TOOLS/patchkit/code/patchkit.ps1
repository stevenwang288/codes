...
function Show-Status {
  ...
  try {
    ...
    Write-Host "[codes-patchkit] repo: $RepoRoot"
    Write-Host "[codes-patchkit] upstream: $remote/$branch"

    $head = (git rev-parse --short HEAD)
    $dirty = (git status --porcelain=v1)
    Write-Host "[codes-patchkit] head: $head"
    if ($dirty) {
      Write-Host "[codes-patchkit] dirty: yes" -ForegroundColor Yellow
    } else {
      Write-Host "[codes-patchkit] dirty: no" -ForegroundColor Green
    }
    ...
    if ($LASTEXITCODE -eq 0 -and $remoteHead) {
      Write-Host "[codes-patchkit] upstream head: $remoteHead"
      ...
      if ($LASTEXITCODE -eq 0) {
        Write-Host "[codes-patchkit] ahead=$ahead behind=$behind"
      }
    }
    ...
    if ($patches.Length -eq 0) {
      Write-Host "[codes-patchkit] patches: none" -ForegroundColor Yellow
    } else {
      Write-Host "[codes-patchkit] patches: $($patches.Length)"
...
function Print-TestInstructions {
  ...
  Write-Host "[codes-patchkit] Home: $home"
  Write-Host "[codes-patchkit] Log:  $log"
  Write-Host "[codes-patchkit] Start: run ./codes (or patchkit start), then open Help/Popular commands." 
  Write-Host "[codes-patchkit] After a few actions, run:"
...
function Do-UpdateWorkflow {
  ...
  try {
    ...
    Write-Host "[codes-patchkit] upstream: $remote/$branch"
    Write-Host "[codes-patchkit] behind: $behind"
    ...
    if ([int]$behind -gt 0) {
      ...
      Write-Host "[codes-patchkit] upstream already up to date." -ForegroundColor Green
    }
  } finally {
    Pop-Location
  }
...
switch ($Cmd) {
  'help' {
    @(
      'codes-patchkit (single entry point)',
      '',
      'Commands:',
      ...
      '  config       Ensure ~/.codes/config.toml hooks (notify/tui.notifications)',
      ...
      '  watchdog     Start watchdog (monitors ~/.codes activity)',
      '',
      'Examples:',
